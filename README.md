...
| 동시 처리 채널 | **8채널** |
| GPU | **RTX 3090 1장** |
| 채널당 분석 주기 | 5 FPS 샘플링 (`PURPOSE_FPS`) |
| 처리 해상도 | 720p 상한 (입력이 더 크면 1280×720으로 축소) |
| 채널당 동시 추론 | TensorRT 엔진 3개 병렬 |
| 재송출 | HLS, x264 `tune=zerolatency` 1500kbps, 2초 세그먼트 |

GPU 1장으로 8채널을 받으려면 채널당 예산이 고정된다. 전 프레임을 추론하지 않고
5 FPS로 샘플링하고 720p로 낮춘 것, 그리고 채널마다 엔진 3개를 **동시에** 돌리는
구조가 이 예산 안에 들어가기 위한 선택이다.
